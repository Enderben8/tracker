...
- Topics are **not** hardcoded: a first-run wizard asks which subjects you take
  and which board examines each, then installs from a catalogue generated from
  the boards' specifications. Never hand-edit `core/.../catalogue/**` — it is
  generated by `tools/spec_tool.py`, and CI verifies every title against the
  board's own words (`docs/NOTES.md`).
